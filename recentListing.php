...
$sql = 'SELECT ' . $title . ', ' . $description . ', ' .PROPERTIES_TABLE . '.* FROM ' . PROPERTIES_TABLE  . ' WHERE approved = 1 ORDER BY id DESC LIMIT '. $conf['recent_limit'];
...
while ($f = $db->fetcharray( $r ))
{
    ...
    $data['garage_cars'] = $f11['garage_cars'];
